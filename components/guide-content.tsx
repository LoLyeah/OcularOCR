...
function Article({ id, locale }: { id: ArticleId; locale: Locale }) {
  ...
  if (id === 'documents') return <div className="space-y-8">
    <Section title={en ? 'Import safely' : 'Impor dengan aman'}>
      <Steps items={en ? ['Select Add Document and choose a PDF, PNG, JPEG, or WebP file up to 25 MB.', 'Review the filename and preview before running OCR.', 'For URL import, use a public HTTPS file URL; private-network and special-purpose addresses are blocked.', 'Wait for storage to finish before closing the tab or locking the vault.'] : ['Pilih Tambah Dokumen dan pilih file PDF, PNG, JPEG, atau WebP hingga 25 MB.', 'Periksa nama file dan pratinjau sebelum menjalankan OCR.', 'Untuk impor URL, gunakan URL file HTTPS publik; alamat jaringan privat dan alamat khusus diblokir.', 'Tunggu penyimpanan selesai sebelum menutup tab atau mengunci vault.']} />
